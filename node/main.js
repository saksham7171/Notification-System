    ...
    MongoClient.connect( url , function (err, db) {
      if (err) {
        ...
        if (request.url === '/favicon.ico') {
            ...
            response.end();
            console.log('favicon requested Served');
            return;
        }
        else if(request.url === '/user'){          // Serving request for current user
        ...
        else if(request.url === '/createUsers'){        //creating dummy users (Configure emails to receive notifications)
            ...
            response.end("Dummy Users Created");
        }
        else if(request.url === '/users'){         // serving request for list of users
        ...
        else if(request.url === '/subscribe'){       //serving request for subscription
             var id;
        ...
        else if(request.url==='/unsubscribe'){          //serving request to unsubscribe
            var id;
            if (request.method == 'OPTIONS') {
                request.on('data', function (data) {
                    id=(JSON.parse(data));
        ...
        else if(request.url==='/update'){                      //serving update request and generate notification
            var id,data;
            if (request.method == 'OPTIONS') {
                request.on('data', function (data) {
